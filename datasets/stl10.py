...
def load(mode='ssl', train=True, bsz=512, root='./data', num_workers=8, cj_str=0.5, original_at='default'):
    ...
    '''
    if mode == 'ssl':
        dataset = STL10(root=root, transform=get_phase1_transform(size, mean, std, cj_str, original_at=original_at),
                                       split='train+unlabeled', download=True)
    
    elif train:
        dataset = STL10(root=root, transform=get_phase2_transform(size, mean, std), download=True)
    
    else:
        dataset = STL10(root=root, transform=get_eval_transform(size, mean, std), split='test', download=True)
